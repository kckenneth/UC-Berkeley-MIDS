...
# My note: Since the `wc -l $data` output is 3736 data/alice.txt
# we strip the text and keep the integer value
...
# default is 1000 lines splitting, we will use customized linesinchunk here
# the ouput will be prefixed with aa, ab, etc
# so here data.chunk.aa, data.chunk.ab, etc 
...
if [ $# -eq 4 ]
  then
    ...
    ################ YOUR CODE HERE #############
    # Q6c solution here:
    # combining all countfiles and piping into a temporary output
    # reducer (collateCounts.py) require 1 argument, so temp output is used
    # remove the temp output afterwards
    ...
    # by piping, we avoid the creation of temporary files
